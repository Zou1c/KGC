...
又一次制作ppt过程中的新发现：
```
现有的高质量模型本身已经具备完成复杂任务的前提下（比如claude opus和google gemini pro），不需要像之前一样人工集成了，只需要给一个模型输入必要信息即可————甚至如果让高智能模型处理其他模型的低质量产物，可能会相比让其直接生成耗费好几倍的token和运行时间（个人感觉以及claude opus4.7自述都指向这个结论）。
